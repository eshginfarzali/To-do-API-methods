...
function getData(){
    ...
    .then(data=> data.todos.forEach(e => {
        ul.innerHTML+=`
            <li  style='${e.completed && 'text-decoration: line-through'}'> ${e.title} <button  onclick='updateTodo(${JSON.stringify(e)})' class="button"><span>Completed</span></button>
             <button onclick='deleteTodo('${e._id}')' class="button"><span>Delete</span></button>
            </li>
        `
